  ...
  static Future getData(
    ...
    try {
      final response = await http.post(
          "http://192.168.43.240/institutions/for_app/get_data/$typeOfData.php",
          body: {
            "$inputData": dataId,
          });
    ...
    if (formKey.currentState.validate()) {
      try {
        ...
        var response = await http.post(
          "http://192.168.43.240/institutions/for_app/insert_data/insert_complant.php",
  ...
  static Future sendTask(GlobalKey<FormState> formKey, BuildContext context,
    ...
    if (formKey.currentState.validate()) {
      try {
        ...
        var response = await http.post(
          "http://192.168.43.240/institutions/for_app/insert_data/insert_task.php",
          body: json.encode(data),
        );
        if (response.statusCode == 200) { // if every things are right then return true
          var message = jsonDecode(response.body);
